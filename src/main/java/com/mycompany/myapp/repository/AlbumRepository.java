...
public interface AlbumRepository extends JpaRepository<Album, Long> {
    ...
    @Query(
        value = "select distinct album from Album album left join fetch album.artist left join fetch album.genre",
        countQuery = "select count(distinct album) from Album album"
    )
    Page<Album> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct album from Album album left join fetch album.artist left join fetch album.genre")
